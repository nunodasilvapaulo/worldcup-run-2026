...
import { callUpRank, getRoster, starterTemplates } from '@/game/players2026'
...
export function NationStartersPanel({ nationId }: { nationId: string }) {
  ...
  const starters = starterTemplates(nationId)
            ...
            <li key={`${nationId}-${p.name}`} className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs text-white truncate leading-tight">
                  <span className="text-[var(--color-wc-gold)]/80 font-bold mr-1">#{callUpRank(p, roster)}</span>
                  {p.name}
                </p>
